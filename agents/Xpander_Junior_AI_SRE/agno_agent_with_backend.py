...
class AgnoAgentWithBackend:
    def __init__(self, agent_backend: AgnoAdapter):
        self.mcp_tools = MultiMCPTools(
            ...
            env={"AWS_ACCESS_KEY_ID": os.environ["PROD_AWS_ACCESS_KEY_ID"], "AWS_SECRET_ACCESS_KEY":
                 os.environ["PROD_AWS_SECRET_ACCESS_KEY"], "AWS_REGION": os.environ["PROD_AWS_REGION"]}
        )
    ...
    async def run(self, message: str, user_id: str, session_id: str, cli: bool = False) -> str:
        ...
        if self.agent is None:
            ...
            self.agent = Agent(
                ...
                tools=[self.mcp_tools, ThinkingTools(
                    add_instructions=True), *self.agent_backend.get_tools()],
